(function (global, factory) {
	...
	function iHam() {
	  ...
	  var board_width = 800;
	  var gene_color;
	  var update_nodes; // external options (exposed API)
	  ...
	  var theme = function theme(div) {
	    d3.select(div).style("position", "relative"); // Mocked data for now...
	    ...
	    .width(tree_width).scale(false)).label(tnt.tree.label.text().fontsize(12).height(config.label_height).text(function (node) {
	      ...
	      update_nodes.call(this, node); // mouse_over_node.display.call(this, node, div)
